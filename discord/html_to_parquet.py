...
def process_all_messages(html_file, output_dir):
    ...
    parquet_file = os.path.join(output_dir, html_file.split(
        '/')[-1].replace('.html', '.parquet'))
...
def main():
    ...
    parser.add_argument('--html_dir', type=str, help='Directory containing HTML files to process',
                        default='data/discord_raw')
    parser.add_argument('--parquet_dir', type=str,
                        help='Directory to write Parquet files to', default='data/discord_parquet')
