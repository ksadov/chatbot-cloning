...
def get_accuracy(client, gt_tsv_questions, gt_tsv_responses, gen_tsv_responses):
    ...
    for i in range(len(gt_tsv_responses)):
        ...
        response = make_anthropic_request(client, messages, "claude-3-opus-20240229")
...
def save_accuracies(
    gen_tsv_file, score_dict, gt_tsv_responses, gen_tsv_responses_all, save_dir
):
    # save as json of response pairs and qa scores, seperate file per gen file
    out_fname = "accuracy_" + os.path.basename(gen_tsv_file)[:-4] + ".json"
    json_list = []
    qa_scores = score_dict
    qa_score_file = os.path.join(save_dir, out_fname)
...
def main():
    ...
    parser.add_argument("--gt_tsv_file", "-gt", default="eval/zef/gt.tsv", type=str)
    parser.add_argument("--gen_tsv_file", "-gen", type=str)
    parser.add_argument("--save_dir", "-s", default="eval/zef/output", type=str)
