    ...
    for i, metadata in enumerate(metadatas):
        ...
        if include_timestamp:
            ...
            if timestamp.year == 1970:
                timestamp = pd.to_datetime(metadata["timestamp"], unit="ms").strftime(
                    "%Y-%m-%d %H:%M"
                )
...
def prep_parquet_documents(document_path, alias_dict, include_timestamp, name):
    ...
    if os.path.exists(os.path.join(document_path, "blog")):
        chat_documents, chat_metadatas = load_parquet_documents(
            os.path.join(document_path, "chat"), alias_dict, include_timestamp, name
        )
    if os.path.exists(os.path.join(document_path, "blog")):
        blog_documents, blog_metadatas = load_parquet_documents(
            os.path.join(document_path, "blog"), alias_dict, include_timestamp, name
        )
    else:
...
def filter_and_chunk(documents, metadatas, chunk_depth, name=None, overlap=0):
    ...
    unique_conversations = list(
        set([metadata["conversation"] for metadata in metadatas])
    )
    for conversation in tqdm(unique_conversations):
        conv_ids = [
            i
            for i, metadata in enumerate(metadatas)
            if metadata["conversation"] == conversation
        ]
...
def format_blog(blog_documents, blog_metadatas, name, include_timestamp):
    ...
    for doc, meta in tqdm(
        zip(blog_documents, blog_metadatas), total=len(blog_documents)
    ):
        meta["user_id"] = name
        if include_timestamp:
            if meta["timestamp"] is not None:
                timestamp = pd.to_datetime(meta["timestamp"]).strftime("%Y-%m-%d %H:%M")
...
def init_HuggingFaceEmbedding(index_info, name, include_timestamp):
    ...
    if not os.path.exists(rag_fname):
        print(f"Creating index at {rag_fname} from {index_info['document_path']}")
        ...
        if index_info["document_path"].endswith(".txt"):
            ...
            documents, metadata, blog_documents, blog_metadata = prep_parquet_documents(
                index_info["document_path"],
                index_info["alias_dict"],
            ...
            chat_documents, chat_metadata = filter_and_chunk(
                documents,
                metadata,
                index_info["chunk_depth"],
                name=name,
                overlap=index_info["overlap"],
            )
            ...
            for i, meta in enumerate(metadata):
                ...
                for m in meta:
                    ...
                    if m["timestamp"] is not None and not isinstance(
                        m["timestamp"], str
                    ):
                        m["timestamp"] = m["timestamp"].strftime("%Y-%m-%d %H:%M")
